									...
									<view class="main-item-wrapper" v-for="(orderItem, ind) in item.orderItems" :key = "ind">
										<view class="disheader-box">
											<view class="item-header">
												...
												<view class="item-header-right" :class="{waitPay: item.status == 2, cancelPay: item.status == 3 || item.status == 4, successOrder: item.status == 5}">{{groupStatus[item.status]}}</view>
											</view>
										</view>
	...
	export default {
		...
		methods: {
			...
			async initOrderList () {
				...
				this.pageNum++
				console.log(this.orderList)
				let res = await this.$fetch(this.$api.orderList,{pageNum: this.pageNum, pageSize: 10, status: this.status, isAssemble: 1}, 'GET', 'FORM')
				console.log(res)
			
				this.orderList = [...this.orderList, ...res.rows]
				
				this.swiperFlag = true // 数据请求完 初始化
