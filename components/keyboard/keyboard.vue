<template>
	<view>
		<view class="tui-keyboard-mask" :class="[show?'tui-mask-show':'']" v-if="cancelOn"></view>
		<view class="tui-keyboard-mask" :class="[show?'tui-mask-show':'']" v-else></view>
			...
			<view class="tui-keyboard-grids">
				...
				<view class="tui-keyboard-grid" :class="[(index==9 || index==11)?'tui-bg-gray':'']" v-for="(item,index) in itemList"
				 :key="index" hover-class="tui-keyboard-hover" :hover-stay-time="150" @tap="handleClick" :data-index="index">
					<view v-if="index<11" class="tui-keyboard-item" :class="[index==9?'tui-fontsize-32':'']">{{getKeyBoard(index,cancelOn)}}</view>
...
<script>
	export default {
		...
		methods: {
			getKeyBoard: function(index, cancelOn) {
				var content = index + 1;
				if (index == 9) {
					content = cancelOn ? "取消" : "清除";
				} else if (index == 10) {
					content = 0;
				}
				return content;
